...
class IdentifyHopFlightScheduleCollectiveWorker
	# include SidekiqStatus::Worker
  # sidekiq_options queue: "identify_flight_schedule_collective_worker", backtrace: true
	def perform()
	  unique_hop_routes = UniqueHopRoute.where(dep_city_code: 'CVG',arr_city_code: 'LON')
    # unique_routes = UniqueHopRoute.where("(dep_country_code='IN' and arr_country_code!='IN') OR (dep_country_code!='IN' and arr_country_code='IN') OR (dep_country_code!='IN' and arr_country_code!='IN') ")
  ...
  def create_schedule_collective(routes,dep_city_code,arr_city_code)
    ...
    routes.each do |route|
  		route_value = InFlightHopScheduleCollective.find_or_create_by(dep_city_code:dep_city_code,arr_city_code: arr_city_code,first_flight_no: route[:flight_no])
