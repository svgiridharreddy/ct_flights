...
class FlightScheduleService
  ...
  def get_airport_deatils
    ...
    if header_record.present?
      flights_header["dep_city_package"] = header_record.dep_city_package == "yes"  ? true : false
      flights_header["arr_city_package"] =  header_record.arr_city_package== "yes" ? true : false
      flights_header["dep_featured_city"] = header_record.dep_city_featured == "yes" ? true : false
      flights_header["arr_featured_city"] = header_record.arr_city_featured == "yes" ? true : false
      flights_header["dep_city_weekend_getaway"] = header_record.dep_city_weekend_getaway == "yes" ? true : false
      flights_header["arr_city_weekend_getaway"] = header_record.arr_city_weekend_getaway == "yes" ? true : false
      flights_header["dep_events_city"] = header_record.dep_city_event == "yes" ? true : false
      flights_header["arr_events_city"] = header_record.arr_city_event == "yes" ? true : false
      flights_header["dep_city_things_to_do"] = header_record.dep_city_things_todo == "yes" ? true : false
      flights_header["arr_city_things_to_do"] = header_record.arr_city_things_todo == "yes" ? true : false
  ...
  def schedule_values(schedule_routes)
    ...
    schedule_layout_values["first_dep_airline"]  =  I18n.t("airlines.#{schedule_routes.first[:carrier_code]}") rescue ""
    schedule_layout_values["first_dep_airline_no"] = schedule_routes.first[:flight_no] rescue ""
    schedule_layout_values["first_dep_time"] = Time.strptime(schedule_routes.first[:dep_time],"%H:%M").to_time.strftime("%I:%M %p") rescue ""
  ...
  def things_to_do
    ...
    if local_activities_cities_list.include?(@arr_city_name.titleize)
      ...
      if local_data.present?
        current_index = local_data.local_activities_current
        if(current_index == 0)
          local_data.update(local_activities_current:current_index+1)
        else
          local_data.update(local_activities_current:current_index+10)
        end
        if local_data.local_activities_current <= local_data.local_activities_total
          if current_index == 0
            activities_data = eval(local_data.local_activities).first(10)
          else
            activities_data = eval(local_data.local_activities).drop(local_data.local_activities_current-10).first(10)
          end
        else
          local_data.update(local_activities_current:0)
          activities_data =  eval(local_data.local_activities).first(10)
        end
        if activities_data.count < 10
          activities_data =  eval(local_data.local_activities).first(10)
        end
      end
    end
  ...
  def from_to_more_routes(url_section,city_section,other_section,city_code)
    ...
    routes = (dir_routes + hop_routes).group_by{|r| [r.dep_city_name,r.arr_city_name]}.values.collect{|a| a.first}
    if @language == "ar"
