...
class FlightTicketService
  ...
  def ticket_hop_values(ticket_routes)
    ...
    ticket_layout_hop_values["first_dep_airline"]  =  I18n.t("airlines.#{ticket_routes.first[:carrier_code]}")
    ticket_layout_hop_values["first_dep_airline_no"] = ticket_routes.first[:flight_no]
    ticket_layout_hop_values["first_dep_time"] = Time.strptime(ticket_routes.first[:dep_time],"%H:%M").to_time.strftime("%I:%M %p")
    ticket_layout_hop_values["last_dep_airline"] = I18n.t("airlines.#{ticket_routes.last[:carrier_code]}")
    ticket_layout_hop_values["last_dep_time"]  = Time.strptime(ticket_routes.last[:dep_time],"%H:%M").to_time.strftime("%I:%M %p")
    ticket_layout_hop_values["last_dep_airline_no"] = ticket_routes.last[:flight_no]
    min_max_duration = ticket_routes.collect{|r| r[:duration]}.minmax
    ticket_layout_hop_values["min_duration"] = if min_max_duration[0].include? (":") then min_max_duration[0].to_time.strftime("%Hh %Mm") else Time.at(min_max_duration[0].to_i*60).utc.strftime("%Hh %Mm") end
    ticket_layout_hop_values["max_duration"] = if min_max_duration[1].include? (":") then min_max_duration[1].to_time.strftime("%Hh %Mm") else Time.at(min_max_duration[1].to_i*60).utc.strftime("%Hh %Mm") end
    ...
    ticket_layout_hop_values["operational_airlines"] = operational_airline_names.to_sentence
    ticket_layout_hop_values["operational_airlines_count"] =  operational_airline_names.count
